...
def main(args):
    ...
    # Setup DDP or single GPU mode:
    if args.single_gpu:
        # Single GPU mode (for debugging/educational purposes)
        rank = 0
        world_size = 1
        device = 0
        ...
        print(f"Starting in single GPU mode: device={device}, seed={seed}")
    else:
        # Distributed mode
    ...
    if args.single_gpu:
        # Single GPU mode: use regular sampler
        sampler = None
        shuffle = True
        # With gradient accumulation, use smaller micro-batch size
        batch_size = args.global_batch_size // args.gradient_accumulation_steps
    else:
        # Distributed mode: use DistributedSampler
    ...
    logger.info(f"Micro-batch size per GPU: {batch_size}, Gradient accumulation steps: {args.gradient_accumulation_steps}")
    logger.info(f"Effective batch size: {args.global_batch_size}")

    # Prepare models for training:
    model_without_ddp = model.module if not args.single_gpu else model
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in loader:
            ...
            accum_iter += 1
            if accum_iter % args.gradient_accumulation_steps == 0:
                scaler.step(opt)
                scaler.update()
                opt.zero_grad()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, default="tiny-imagenet-200/train")
